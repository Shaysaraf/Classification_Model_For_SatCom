...
class SatComDataManager:
    def __init__(self):
        # --- PATHS ---
        # Use env vars for portability (GCP/Linux vs Windows) with safe defaults
        self.input_dir = Path(os.environ.get('DATA_DIR', '/data/tempo/iq_augmented_cut'))
        self.output_dir = Path(os.environ.get('OUTPUT_DIR', '/app/results'))
        self.master_json_path = Path(os.environ.get('METADATA_FILE', '/app/data_ready_SR.json'))
        self.extension = ".iq"
